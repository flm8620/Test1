...

{
...

	{
		...
		data[index_facet*number_of_class + 0] = f->area() * double(f->z()-zmin)/(zmax-zmin); //yellow //sol

		data[index_facet*number_of_class + 1] = f->area() * (1.0-double(f->z()-zmin)/(zmax-zmin)); //blue //roof

		data[index_facet*number_of_class + 2] = f->area() * fabs(f->normal().z()); //facade white
	...
	for (int l2 = 0; l2 < number_of_class; l2++){
		...
		smooth[l1 + l2*number_of_class] = l1==l2 ? 0 : 1;
...

{
...

	{
...

		{
			...
			class_index++; std::cout<<"class "<<class_index<<std::endl;
			...
			do{
				...
				for (int k = 0; k<(int)index_container_former_ring.size(); k++){
					...
					int count = 0; //std::cout<<"look at face "<<point_index<<std::endl;
...

					{
						...
						if(label_region[neighbor_index]!=-1) continue;

						//std::cout<<"a neighor "<<neighbor_index;

						//-------------- TO FILL IN (Start)---------------------

						//Facet_iterator face = list_facets[point_index];

						//Vector_Flo normal_face = face->normal();

						Vector_Flo normal = (*it)->normal();

						//Point_Flo pt = (*it)->center();



						if(std::cos(angle_max)<normal*normal_seed){
							...
							//std::cout<<", put in class "<<class_index<<std::endl;
